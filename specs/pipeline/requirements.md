...
- Clase `tempifyPipeline` con método `run(source) -> PipelineResult` (ver Capa 5 en `../../steering/architecture.md`).
...
WHEN a `ProgressCallback` is provided in `PipelineConfig`, THE SYSTEM SHALL invoke it at the start and end of each of the seven phases and, during interpolation, at a configurable frequency (default every 5% of processed pixels) with the current phase name and progress fraction.
...
WHERE the caller passes `dry_run=True` in `PipelineConfig`, THE SYSTEM SHALL execute phases `detect`, `validate_geospatial`, and `validate_compatibility` only, return a `PipelineResult` with `outputs=[]`, and skip interpolation, writing, and post-validation.
